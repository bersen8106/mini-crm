                ...
                <div class="overflow-hidden overflow-x-auto p-6 bg-white border-b">
                    ...
                    <form method="POST" action="{{ route('tasks.update', $task) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="description" class="block mt-1 w-full" type="text" name="description" :value="old('description', $task->description)" required autofocus autocomplete="description" />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="deadline_at" class="block mt-1 w-full" type="date" name="deadline_at" min="{{ today()->format('Y-m-d') }}" :value="old('deadline_at', $task->deadline_at)" required autocomplete="deadline" />
                            ...
                            <select name="client_id" class="block mb-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" id="client_id">
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}"
                                        @selected(old('client_id', $client->client_id) == $client->id)>{{ $client->company_name }}</option>
                                @endforeach
                            </select>
                            ...
                            <select name="project_id" class="block mb-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" id="project_id">
                                @foreach($projects as $project)
                                    <option value="{{ $project->id }}"
                                        @selected(old('project_id', $project->client_id) == $project->id)>{{ $project->title }}</option>
                                @endforeach
                            </select>
